...
module.exports = async (req, res) => {
  ...
  // GET sem body = retorna a página HTML
  if (req.method === 'GET' && !req.body) {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.status(200).send(HTML_PAGE);
  }

  // GET com accept: application/json = retorna status do banco
  ...
  // GET genérico = retorna a página HTML (fallback)
  if (req.method === 'GET') {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.status(200).send(HTML_PAGE);
  }
  ...
  try {
    ...
    if (values.length === 0) {
      return res.status(400).json({ error: 'Nenhuma linha válida.' });
    }
